...
const hexToBytes = (Ntag424.util.hexToBytes = (hex: any) => {
    let bytes = [];
...
const bytesToHex = (Ntag424.util.bytesToHex = (bytes: any) => {
    let hex = [];
...
function leftRotate(bytesArr: any, rotatebit: any = 1) {
...
//Encrypted IV
function ivEncryption(ti: any, cmdCtr: any, sesAuthEncKey: any) {
...
function ivEncryptionResponse(ti: any, cmdCtr: any, sesAuthEncKey: any) {
    const ivData = AES.encrypt(
...
function padForEnc(data: any, byteLen: any) {
 ...
 * @returns
 */
function decToHexLsbFirst(dec: any, bytes: any) {
    //lsb first
    return dec
        .toString(16)
        .padStart(2, "0")
 ...
 */
Ntag424.sendAPDUCommand = async function (commandBytes: any) {
    const response =
        Platform.OS == "ios"
 ...
 * CommMode N/A
 */
Ntag424.AuthEv2First = async function (keyNo: any, pKey: any) {
    ...
    try {
        ...
        if (resultCode == "91af") {
            ...
            console.warn("Result: ", bytesToHex([secondAuthRes.sw1, secondAuthRes.sw2]));
            ...
            if (secondAuthResultCode == "9100") {
                ...
                return Promise.reject(
                    "Auth Failed, code " + secondAuthResultCode + " " + errorCodes[secondAuthResultCode]
                );
            }
        } else {
            //auth failed
 ...
 * @returns
 */
Ntag424.AuthEv2NonFirst = async (keyNo: any, pKey: any) => {
    const bytes = hexToBytes("9077000001" + keyNo + "00");
    const Result = await Ntag424.sendAPDUCommand(bytes);
    console.warn("auth ev2 non first part 1 Result: ", bytesToHex([Result.sw1, Result.sw2]));
    const resultData = bytesToHex(Result.response);
    //91AF is the successful code
    const resultCode = bytesToHex([Result.sw1, Result.sw2]);
    if (resultCode == "91af") {
        ...
        const secondAuthBytes = hexToBytes("90AF000020" + RndARndBEnc + "00");
        const secondAuthRes = await Ntag424.sendAPDUCommand(secondAuthBytes);
        console.warn("auth ev2 non first part 2 Result: ", bytesToHex([secondAuthRes.sw1, secondAuthRes.sw2]));
        //9100 is the successful code
        const secondAuthResultCode = bytesToHex([secondAuthRes.sw1, secondAuthRes.sw2]);
        if (secondAuthResultCode == "9100") {
            //auth successful
 ...
 * @returns
 */
Ntag424.calcMac = function (commandData: any) {
 ...
 * @returns
 */
Ntag424.encData = function (cmdDataPadd: any, cmdCtr: any) {
 ...
 * @returns
 */
Ntag424.setBoltCardFileSettings = (piccOffset: any, macOffset: any) => {
 ...
 * @returns
 */
Ntag424.changeFileSettings = async (cmdData: any) => {
    ...
    if (resCode == "9100") {
        ...
        return Promise.reject(
            "Change file settings Failed, code " + resCode + " " + changeFileSettingsErrorCodes[resCode]
        );
    }
};
 ...
 * @returns
 */
Ntag424.changeKey = async (keyNo: any, key: any, newKey: any, keyVersion: any) => {
    const cmdCtr = decToHexLsbFirst(Ntag424.cmdCtrDec++, 2);
    ...
    if (resCode == "9100") {
        ...
        return Promise.reject(
            "Change Key Failed for key " +
                parseInt(keyNo, 16) +
                ", code " +
                resCode +
                " " +
                changeKeyErrorCodes[resCode]
        );
    }
};
 ...
 * @returns
 */
Ntag424.setNdefMessage = async (ndefMessageByte: any) => {
    try {
        await Ntag424.isoSelectFileApplication();

        const secondISO = await Ntag424.sendAPDUCommand(hexToBytes("00A4000002E10300"));
        console.log("2nd isoSelectRes: ", bytesToHex([secondISO.sw1, secondISO.sw2]));

        const isoSelectFileBytes = hexToBytes("00A4000002E10400");
        const isoSelectRes = await Ntag424.sendAPDUCommand(isoSelectFileBytes);
        console.log("3rd isoSelectRes: ", bytesToHex([isoSelectRes.sw1, isoSelectRes.sw2]));
        const resultHex = bytesToHex([isoSelectRes.sw1, isoSelectRes.sw2]);
        if (resultHex == "9000") {
        } else {
        ...
        console.log("isoUpdateBinaryRes Res: ", resCode);
        if (resCode == "9000") {
 ...
 * @returns
 */
Ntag424.readData = async (offset: any) => {
    ...
    console.warn("readData Res: ", resCode, resData);
    if (resCode == "9100") {
 ...
 * @returns
 */
Ntag424.isoReadBinary = async (offset: any) => {
    await Ntag424.isoSelectFileApplication();

    const isoSelectFileBytes = hexToBytes("00A4000002E10400");
    const isoSelectRes = await Ntag424.sendAPDUCommand(isoSelectFileBytes);
    console.warn("isoSelectRes: ", bytesToHex([isoSelectRes.sw1, isoSelectRes.sw2]));
    const resultHex = bytesToHex([isoSelectRes.sw1, isoSelectRes.sw2]);
    if (resultHex == "9000") {
    } else {
        return Promise.reject("ISO Select File Failed, code " + resultHex + " " + errorCodes[resultHex]);
    ...
    console.warn("isoReadBinary Res: ", resCode, resData);
    if (resCode == "9000") {
        return Promise.resolve(resData);
    } else {
 ...
 * @returns
 */
Ntag424.getKeyVersion = async (keyNo: any) => {
 ...
 * @returns
 */
Ntag424.setConfiguration = async (option: any, configData: any) => {
 ...
 * @returns
 */
Ntag424.testPAndC = async (pVal: any, cVal: any, uid: any, piccKey: any, macKey: any) => {
    ...
    const sesSdmFileReadMAC = CryptoJS.CMAC(CryptoJS.enc.Hex.parse(macKey), CryptoJS.enc.Hex.parse(sv2));
    const sesSdmFileReadMACHex = sesSdmFileReadMAC.toString();
    const sdmMac = CryptoJS.CMAC(CryptoJS.enc.Hex.parse(sesSdmFileReadMACHex));
    const sdmMacHex = sdmMac.toString();

    const truncatedMacBytes = hexToBytes(sdmMacHex).filter(function (element, index, array) {
        return (index + 1) % 2 === 0;
    });
